#!/usr/bin/env python3
"""
Synchronise prestations.json vers Prestafacture /produits (payload catalogue complet).
...
Usage:
  python scripts/prestafacture_sync_prestations.py
  python scripts/prestafacture_sync_prestations.py --full          # PATCH complet (sans supprimer)
  python scripts/prestafacture_sync_prestations.py --recreate    # DELETE + POST complet
  python scripts/prestafacture_sync_prestations.py --slug site-vitrine --recreate
  python scripts/prestafacture_sync_prestations.py --dry-run --recreate
"""
...
from prestafacture_product_payload import build_rich_product_body, livrables_summary

ROOT = Path(__file__).resolve().parents[1]
PRESTATIONS_JSON = ROOT / "src" / "data" / "prestations.json"

SERVICE_SLUG_TO_SKU: dict[str, str] = {
    "pack_vitrine_essentiel": "SITE-VITRINE-ESSENTIEL",
...
def api_request(method: str, path: str, body: dict | None = None) -> tuple[int, dict | str]:
    ...
    if not base or not token:
        print("PRESTAFACTURE_API_BASE ou PRESTAFACTURE_API_TOKEN manquant dans .env", file=sys.stderr)
        sys.exit(1)
...
class ProductCatalog:
    ...
    def load(self) -> None:
        ...
        while True:
            ...
            if status == 401:
                raise RuntimeError(
                    "Jeton Prestafacture invalide (401). Regenerez fact_… dans Parametres → API."
                )
            if status == 429:
...
def resolve_sku(entry: dict, *, is_addon: bool) -> str:
    explicit = (entry.get("prestafacture_sku") or "").strip()
    if explicit:
...
def sync_entry(
    ...
    entry["prestafacture_sku"] = sku
    if pid is not None:
        entry["prestafacture_product_id"] = pid
        print(f"    id={pid} ({action})")
        return True
    if dry_run:
        entry.pop("prestafacture_product_id", None)
        print(f"    ({action})")
    return dry_run


def main() -> int:
    parser = argparse.ArgumentParser(description="Sync prestations.json -> Prestafacture produits (complet)")
    ...
    catalog = ProductCatalog()
    if not args.dry_run:
        print("Chargement catalogue Prestafacture (/produits)...")
        try:
            catalog.load()
    ...
    if args.recreate or args.full:
        print("Rafraichissez le catalogue Prestafacture (F5).")
