#!/usr/bin/env python3
"""Test API Facturio (lecture .env local). Usage: python scripts/facturio_test.py [--devis]"""
...
def api_request(method: str, path: str, body: dict | None = None) -> tuple[int, dict | str]:
    ...
    if not base or not token:
        print("FACTURIO_API_BASE ou FACTURIO_API_TOKEN manquant dans .env")
        sys.exit(1)
...
def catalog_product_id(slug: str = "site-vitrine") -> int | None:
    """Lit facturio_product_id depuis src/data/prestations.json (sync locale ou prod)."""
    ...
    for item in data.get("items") or []:
        if isinstance(item, dict) and item.get("slug") == slug:
            pid = item.get("facturio_product_id")
...
def main() -> None:
    ...
    if product_id:
        ...
        print("productId absent — ligne libre (lancez facturio_sync_prestations.py)")

    devis_body = {
        "clientId": client_data["id"],
        "expiryDate": "2026-09-01",
        "notes": "Test script facturio_test.py — à archiver",
        "lines": [line],
    }
