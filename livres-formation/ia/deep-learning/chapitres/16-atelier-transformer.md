...
Objectif : relier architecture et gestes d'usage. Duree : 35 minutes.

## Etapes

1) Explique l'attention avec ton propre exemple de phrase. 2) Mesure mentalement une limite de contexte : que coupes-tu dans un long PDF ? 3) Concevoir un mini-RAG : 5 documents, question, extrait a coller, consigne anti-invention. 4) Deux temperatures mentales sur la meme tache (strict vs creatif) et grille d'evaluation. 5) Decrire un fine-tuning que tu NE feras pas maintenant, et pourquoi prompting/RAG suffisent. 6) Lister 3 risques (donnees, hallucination, cout tokens).

## Livrable

Fiche "du transformer a mon usage" avec schema attention + protocole RAG manuel + regles de cout.
## Budget tokens

Estime le cout mensuel d'un usage : questions/jour * tokens moyens * prix. Ajoute le cout de tes erreurs (temps de verification). Decide si un modele plus petit suffit pour 80 % des cas.
