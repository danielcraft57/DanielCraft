...
def inject_figures(
    ...
    *,
    ...
    schemas = [(src, cap) for src, cap in figures if not str(src).startswith("js-scene-")]
    scenes = [(src, cap) for src, cap in figures if str(src).startswith("js-scene-")]
    ...
    if scenes:
        ...
        for anchor in ("Petite histoire", "Erreur classique", "En vrai", "A toi"):
            ...
            if re.search(pattern, body_html, flags=re.I):
                ...
                break
        if not placed:
            # Fallback : apres le premier callout, sinon apres le schema / H1
            body_html, n = re.subn(
                r"(</aside>)",
                r"\1\n" + scene_block,
                body_html,
