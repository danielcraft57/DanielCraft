# Quiz final

## Question 1
Une blockchain, c'est surtout :
- A) Une banque centrale
- B) Un registre partage en chaines de blocs
- C) Un livret garanti

## Question 2
"Not your keys, not your coins" rappelle :
- A) Que la seed est inutile
- B) Que sans cles, tu depends souvent d'un tiers
- C) Que BTC ne baisse jamais

## Question 3
Une seed phrase :
- A) Se partage au support Discord
- B) Doit rester secrete / offline
- C) Se post sur Instagram

## Question 4
Apres -50 %, pour revenir il faut environ :
- A) +50 %
- B) +100 %
- C) +10 %

## Question 5
Un stablecoin :
- A) Est toujours exactement du cash bancaire
- B) Vise une parite, avec risques d'emetteur / depeg
- C) Remplace la poche B

## Question 6
La DeFi expose surtout a :
- A) Zero risque
- B) Smart contracts, liquidite, incentives
- C) Uniquement la fiscalite

## Question 7
Sur un exchange, un risque majeur est :
- A) La couleur du logo
- B) Le risque de contrepartie / retrait
- C) L'absence totale de frais jamais

## Question 8
Signal d'arnaque :
- A) Export CSV
- B) "Entrez votre seed pour valider"
- C) Lire une alerte AMF

## Question 9
Gas sur Ethereum :
- A) Est un pourboire optionnel sans effet
- B) Est le cout d'execution a surveiller avant signature
- C) Garantit le profit

## Question 10
Pour un debutant prudent, le levier futures :
- A) Est obligatoire
- B) Est generalement a eviter
- C) Annule la volatilite

## Question 11
Journal des operations sert surtout a :
- A) Faire joli
- B) Suivi et hygiene fiscale / clarte
- C) Augmenter le levier

## Question 12
Plafond % patrimoine crypto :
- A) Inutile
- B) Outil central de discipline
- C) Fixe par Twitter

## Corrige

1B, 2B, 3B, 4B, 5B, 6B, 7B, 8B, 9B, 10B, 11B, 12B.

:::retenir
Cles, plafonds, journal, non au phishing.
:::

:::attention
Sans-faute != signal d'acheter.
:::

:::astuce
Refais 2, 4, 8.
:::
